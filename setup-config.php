...
<body class="wp-core-ui" style="position: relative">
	...
	<?php
		...
		if ($php != '') {
			$sql = "UPDATE table_conversion_config SET php_cli_path=$php WHERE id=1";
			$conn->query($sql);
		}

		$ffmpeg = trim(shell_exec('which ffmpeg'));
		if ($ffmpeg == '') {
			shell_exec('./install_ffmpeg.sh');
			$ffmpeg = trim(shell_exec('which ffmpeg'));
		}
		$ffmpeg = trim(shell_exec('which ffmpeg'));
		$sql = "UPDATE table_conversion_config SET ffmpeg_path=$ffmpeg WHERE id=1";
		$conn->query($sql);
	?>
		<p><h1>Configuration required libraries for xStreamer Lite</h1></p>
		<ul class="configuration">
			<li>Configured PHP</li>
			<li>Configured Mplayer</li>
			<li>Configured Mencoder</li>
			<?php if ($ffmpeg != '') { ?>
				<li>Configured FFMPEG</li>
			<?php } else { ?>
				<li class="error">FFMPEG Not Found</li>
			<?php } ?>
			<li>Configured FLV tool2</li>
			<li>Configured MP4Box</li>
			<li>Configured Mediainfo</li>
			<li>Configured Yamdi</li>
		</ul>
		<?php if ($ffmpeg == '') { ?>
			<p>The FFMPEG not available in this host. You can request your host provider to install FFMPEG.</p>
		<?php } ?>
		<p class="step"><a href="<?php echo "http://" . $_SERVER["SERVER_NAME"]; ?>" class="button button-large">Completed</a></p>
	<?php
