        ...
        <tbody> 
            ...
            @foreach($message as $result)
                <tr> 
                    ...
                    <td class="line_table">
                        <a target="_new" href="{{URL()}}/{{$result->post_name}}.{{$result->string_Id}}.html"> 
                            #{{$result->string_Id}} / {{$result->title_name}}
                        </a>
                    </td> 
                    ...
                    <td class="line_table">
                        <?=($result->status==0)? "<span class='label label-info'>New message</span>" :"<span class='label label-success'>Replied</span>" ?>
                    </td>
                    <td>
                        <a href="{{URL('admincp/reply-message/')}}/{{$result->ID}}">
                            <i style="font-size: 20px ; margin: 5px" class="fa fa-pencil-square-o"></i>
                        </a>
                        <a href="{{URL('admincp/delete-message')}}/{{$result->ID}}" onclick="return confirm('Are you sure you want to delete MessageID: {{$result->ID}} ?')">
                            <i style="font-size: 20px ; margin: 5px" class="fa fa-trash-o"></i>
                        </a>
                    </td>
                </tr> 
            @endforeach
